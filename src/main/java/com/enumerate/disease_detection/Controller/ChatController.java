...
import com.enumerate.disease_detection.Utils.Tools.TitleTool;
...
public class ChatController {
    ...
    public SseEmitter memory(HttpServletResponse response, @RequestParam String prompt, @RequestParam Long userId, @RequestParam Long sessionId) {
        // ========== 1. 配置【流式必备的3个核心响应头】，缺一不可 ==========
        response.setContentType("text/event-stream;charset=UTF-8"); // 声明SSE流式格式
        response.setHeader("Cache-Control", "no-cache"); // 禁止客户端缓存，防止分片错乱
        response.setHeader("X-Accel-Buffering", "no"); // 禁止nginx反向代理缓冲，部署必加
        ...
        // ========== 2. 创建流式发射器，设置超时时间60秒（按需调整） =========
        SseEmitter emitter = new SseEmitter(60 * 1000L);
        chatService.memory(emitter, prompt, userId, sessionId);
        return emitter;
    }
